...
discountSchema.virtual("isCurrentlyValid").get(function() {
  const now = new Date();
  return this.discount_is_active &&
         now >= this.discount_start_date &&
         now <= this.discount_end_date &&
         (this.discount_max_uses === null || this.discount_uses_count < this.discount_max_uses);
});
...
discountSchema.index({ discount_code: "text", discount_name: "text", discount_description: "text" });
...
discountSchema.pre("save", function(next) {
  ...
  if (this.discount_type === DiscountType.PERCENTAGE && this.discount_value > 100) {
  ...
  if (this.discount_type === DiscountType.FIXED_AMOUNT &&
    ...
    return next(new Error("Minimum order value should be at least equal to fixed discount amount"));
  }
...
discountSchema.statics.findByCode = function(code: string, shopId: Types.ObjectId) {
  return this.findOne({
...
discountSchema.methods.canBeUsedBy = function(userId: Types.ObjectId) {
  ...
  const userUsageCount = this.discount_users_used.filter(id => id.equals(userId)).length;
